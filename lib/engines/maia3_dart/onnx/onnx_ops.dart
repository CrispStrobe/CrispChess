...
Tensor _elementwiseBinary(Tensor a, Tensor b, double Function(double, double) op) {
...
Tensor opPow(Tensor a, Tensor b) => _elementwiseBinary(a, b, (x, y) => math.pow(x, y).toDouble());
...
Tensor opShape(Tensor x) => Tensor.int64(Int64List.fromList(x.shape), [x.shape.length]);
...
Tensor opSqueeze(Tensor x, List<int>? axes) {
  final ax = (axes ?? [for (int k = 0; k < x.shape.length; k++) if (x.shape[k] == 1) k])
      .map((a) => a < 0 ? a + x.shape.length : a)
      .toSet();
  final newShape = [for (int k = 0; k < x.shape.length; k++) if (!ax.contains(k)) x.shape[k]];
  return x.isFloat ? Tensor.float(x.f!, newShape) : Tensor.int64(x.i!, newShape);
}
...
Tensor opConcat(List<Tensor> inputs, int axis) {
  ...
  return isFloat ? Tensor.float(outF!, outShape) : Tensor.int64(outI!, outShape);
}
...
Tensor opGather(Tensor data, Tensor indices, int axis) {
  ...
  final outShape = [...data.shape.sublist(0, ax), ...idxShape, ...data.shape.sublist(ax + 1)];
...
Tensor opSlice(Tensor x, List<int> starts, List<int> ends, List<int>? axes, List<int>? steps) {
  ...
  for (int a = 0; a < rank; a++) {
    final cnt = normStep[a] > 0
        ? math.max(0, (normEnd[a] - normStart[a] + normStep[a] - 1) ~/ normStep[a])
        : math.max(0, (normStart[a] - normEnd[a] - normStep[a] - 1) ~/ (-normStep[a]));
    outShape.add(cnt);
  }
...
Tensor opReduceMean(Tensor x, List<int>? axes, bool keepdims) {
  ...
  final outShapeFull = [for (int k = 0; k < rank; k++) ax.contains(k) ? 1 : x.shape[k]];
  ...
  final squeezedShape = [for (int k = 0; k < rank; k++) if (!ax.contains(k)) x.shape[k]];
...
Tensor opLayerNormalization(Tensor x, Tensor scale, Tensor bias, int axis, double epsilon) {
  final rank = x.shape.length;
  final ax = axis < 0 ? axis + rank : axis;
  final outerSize = x.shape.sublist(0, ax).fold<int>(1, (a, b) => a * b);
...
Tensor opMatMul(Tensor a, Tensor b) {
  ...
  final af = a.f, bf = b.f; // non-null in the (only actually occurring) all-float case

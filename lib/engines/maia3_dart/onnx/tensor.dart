...
class Tensor {
  ...
  factory Tensor.scalarFloat(double v) => Tensor.float(Float32List.fromList([v]), const []);
  factory Tensor.scalarInt(int v) => Tensor.int64(Int64List.fromList([v]), const []);
  ...
  void _checkLength(int len) {
    ...
    assert(len == expected, 'Tensor data length $len != product of shape $shape ($expected)');
  }
  ...
  Tensor reshape(List<int> newShape) {
    ...
    if (negIdx != -1) {
      final knownProduct = resolved.where((d) => d != -1).fold<int>(1, (a, b) => a * b);

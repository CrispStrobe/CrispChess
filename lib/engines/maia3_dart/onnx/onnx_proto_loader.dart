...
Tensor tensorFromProto(TensorProto t) {
  ...
  if (t.dataType == _kFloat) {
    ...
    if (t.int64Data.isNotEmpty) {
      return Tensor.int64(Int64List.fromList(t.int64Data.map((v) => v.toInt()).toList()), shape);
    }
    final bytes = Uint8List.fromList(t.rawData);
    final bd = ByteData.sublistView(bytes);
  ...
  throw UnsupportedError('Unsupported ONNX TensorProto dataType ${t.dataType} for "${t.name}"');
}

...
class OnnxGraphExecutor {
  ...
  Map<String, Tensor> run(Map<String, Tensor> inputs, List<String> outputNames) {
    ...
    for (final node in _graph.node) {
      ...
      final ins = [for (final name in node.input) name.isEmpty ? null : values[name]];
      List<Tensor> outs;
      try {
  ...
  List<Tensor> _dispatch(String opType, List<Tensor?> ins, _AttrMap attrs) {
    ...
    switch (opType) {
      ...
      case 'Clip':
        return [ops.opClip(need(0), ins.length > 1 ? ins[1] : null, ins.length > 2 ? ins[2] : null)];
      case 'Cast':
      ...
      case 'Squeeze':
        return [ops.opSqueeze(need(0), ins.length > 1 ? ins[1]!.asIntList() : null)];
      case 'Unsqueeze':
        return [ops.opUnsqueeze(need(0), need(1).asIntList())];
      case 'Concat':
        return [ops.opConcat([for (final t in ins) t!], attrs.getInt('axis')!)];
      case 'Gather':
        return [ops.opGather(need(0), need(1), attrs.getInt('axis') ?? 0)];
      case 'GatherND':
        return [ops.opGatherND(need(0), need(1), attrs.getInt('batch_dims') ?? 0)];
      case 'Expand':
      ...
      default:
        throw UnsupportedError('ONNX op "$opType" not implemented in the Dart interpreter');
    }
  }
}
...
class _AttrMap {
  ...
  _AttrMap(Iterable<AttributeProto> attrs) : _byName = {for (final a in attrs) a.name: a};

  int? getInt(String name) => _byName.containsKey(name) ? _byName[name]!.i.toInt() : null;
  double? getFloat(String name) => _byName.containsKey(name) ? _byName[name]!.f : null;
  String? getString(String name) =>
      _byName.containsKey(name) ? String.fromCharCodes(_byName[name]!.s) : null;
  List<int>? getInts(String name) =>
      _byName.containsKey(name) ? _byName[name]!.ints.map((v) => v.toInt()).toList() : null;
}

...
| **ChessMamba** | MIT | ~800 | All | State-space net reading the move sequence, own search; downloaded (67 MB) |
| **Searchless 9M / 136M / 270M** | CC BY 4.0 (weights) | up to ~2900 (270M, paper) | All (9M) / Desktop | DeepMind's transformers that play without any search |
| **LM bot zoo** | MIT / Apache-2.0 (per model) | ~400 | All (4) / Desktop (8) | Eight small chess language models from the Hub, downloaded on demand |
...
- **ChessMamba** — [TobiasLogic's](https://huggingface.co/TobiasLogic/chessmamba) selective state-space network (Mamba/S6, 16.8M parameters). It never sees the board: it reads the game as a sequence of moves and carries a fixed-size state from move to move, so a move costs the same at move 5 as at move 50. Played with its own policy-guided negamax, expanding a node's candidate moves in one batched call. Games from the starting position only; others go to the built-in engine. Measured about 800 against Maia anchored at 1100 (84-game round robin); its search adds little over the network's first choice.
- **Searchless 9M / 136M / 270M** — Google DeepMind's "Grandmaster-Level Chess Without Search" transformers (weights CC BY 4.0). No lookahead at all: the network judges every legal move once, in one batch, and plays the one with the highest predicted win probability. The 270M model reached a Lichess blitz rating of about 2895 in the paper. Converted to ONNX and checked against the original JAX engine (same moves; win probabilities within 1e-6 for 9M).
- **LM bot zoo** — eight small chess *language models* from the Hugging Face Hub (see NOTICE.md for authors), each exported with a KV cache and tokenized in Dart exactly as upstream. A bot writes the game as PGN text and picks among the legal moves by how likely it finds each one's text, so it can never play an illegal move. Given the real game, the best of them (Chess Llama 68M) plays the move a human played in 41% of positions from real Lichess games, against 1-9% for the blind prompt the Chess LLM Arena uses. In play they are weak: about 400 against Maia anchored at 1100 (they never won a game there). Odd, human-ish opponents, not engines.
...
Stockfish uses below, since it really does run GPL code, just
